...
try:
    with sqlite3.connect("Registro_venta.db") as conec:
        c = conec.cursor()
        c.execute("CREATE TABLE IF NOT EXISTS totales (clave INTEGER PRIMARY KEY, total TEXT NOT NULL);")
        c.execute("CREATE TABLE IF NOT EXISTS venta (clave INTEGER PRIMARY KEY, descripcion TEXT NOT NULL, unidades INTEGER NOT NULL ,   precio TEXT NOT NULL, fecha DATE,n_cuenta INTEGER NOT NULL);")
